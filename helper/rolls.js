module.exports = { rollValue, tableRoll };

// return the applicable JSON row for a roll on a table
function tableRoll(diceRoll, tableArray) {
  const rollResult = rollValue(diceRoll);

  for (let i = 0; i < tableArray.length; i++) {
    if (rollResult <= tableArray[i].maxRoll) return tableArray[i];
  }
}
...
function rollSingleValue(inputVal) {
  ...
  if (dPosition < 0) {
    ...
    if (Number.isNaN(totalDice) || Number.isNaN(diceSize))
      throw "invalid input";
    let totalVal = 0;
    let diceRoll = 0;
